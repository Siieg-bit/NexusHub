-- Migration 192: Corrigir erro ao curtir (violação de not-null na coluna title de notifications)
-- A coluna title é obrigatória em notifications, mas a RPC não a enviava.
-- Também tornamos a coluna title opcional na tabela para maior flexibilidade futura.

-- 1. Tornar a coluna title opcional
ALTER TABLE public.notifications ALTER COLUMN title DROP NOT NULL;

-- 2. Atualizar a RPC toggle_reaction_with_reputation para enviar um título padrão caso necessário
-- (Embora agora seja opcional, é boa prática enviar para manter a consistência visual das notificações)
...
BEGIN
  ...
  IF v_existing_type IS NULL THEN
    ...
    IF p_type IN ('like', 'love') AND v_target_author != p_user_id THEN
      ...
      INSERT INTO public.notifications (user_id, actor_id, type, post_id, title, content)
      VALUES (
        v_target_author, 
        p_user_id, 
        'like', 
