-- ============================================================
-- Migration 216: Corrigir todas as RPCs admin com os nomes
-- de colunas corretos do banco real.
-- DROP das funções existentes para permitir mudança de assinatura
...
-- Erros encontrados:
--   - p.avatar_url → p.icon_url (tabela profiles)
--   - is_active não existe em profiles (só em ai_characters)
--   - moderation_logs não tem actor_id separado, usa moderator_id
-- ============================================================
...
BEGIN
  ...
  SELECT
    ...
    p.is_team_admin,
    p.is_team_moderator,
    false AS is_banned,  -- profiles não tem is_banned, verificar em bans
    p.created_at
  FROM profiles p
  WHERE
...
BEGIN
  ...
  SELECT team_rank INTO v_target_rank FROM profiles WHERE id = p_target_id;
  ...
  -- Verificar hierarquia: só pode atribuir cargos menores que o seu
  IF v_new_rank >= v_caller_rank THEN
    RAISE EXCEPTION 'Você não pode atribuir um cargo igual ou superior ao seu';
  END IF;

  -- Verificar se pode modificar o alvo
  IF v_target_rank >= v_caller_rank THEN
    RAISE EXCEPTION 'Você não pode modificar um membro de rank igual ou superior';
  END IF;

  -- Aplicar o novo cargo
  UPDATE profiles
  SET team_role = p_role::team_role
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'active_communities', (SELECT COUNT(*) FROM communities WHERE status = 'active'),
...
BEGIN
  ...
  WHERE (p_source IS NULL OR ct.source::text = p_source)
...
BEGIN
  ...
  -- Verificar se a tabela lottery_logs existe
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'lottery_logs' AND table_schema = 'public') THEN
