...
| P10.1 | APK/AAB | Medir tamanho baseline antes de reduzir. | Automação criada e executada no sandbox; baseline real pendente de CI/dev com Flutter | Script `scripts/measure_android_binary_size.py`, relatório `docs/APK_SIZE_BASELINE.md` e artefatos `android-size-debug`/`android-size-release` no CI foram adicionados. No sandbox não há Flutter/Dart nem APK/AAB em `frontend/build/app/outputs`; assets locais medidos: 0,324 MiB total, 0,082 MiB imagens e 0,242 MiB fontes. |
| P10.2 | APK/AAB | Remover payloads redundantes apenas após estabilidade. | Não iniciado; bloqueado por baseline real e validação Flutter/device | Nenhuma remoção, minificação ou `shrinkResources` foi ativado nesta etapa; redução real só deve ocorrer após relatório comparável de APK split por ABI e AAB release. |
