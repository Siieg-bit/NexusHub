...
function AminoMainHeader({ onSearchClick, onBack, title }: { onSearchClick?: () => void; onBack?: () => void; title?: string }) {
  ...
  return (
    <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }}
      className="flex items-center gap-2.5 px-3 py-2 bg-[#0f0f1e]/95 backdrop-blur-sm border-b border-white/5 shrink-0" style={{ paddingTop: 38 }}>
      {onBack ? (
        <button onClick={onBack} className="p-1 active:scale-90 transition-transform"><ChevronLeft size={22} className="text-white" /></button>
      ) : (
        <button onClick={() => navigateTo("profile")} className="active:scale-95 transition-transform shrink-0">
          <img src={currentUser.avatar} className="w-9 h-9 rounded-full object-cover border-2 border-white/15" alt="" />
        </button>
      )}
      {title ? (
        <span className="text-white font-bold text-[15px] flex-1 truncate">{title}</span>
      ) : (
        <button onClick={onSearchClick || (() => comingSoon("Search"))}
          className="flex-1 flex items-center bg-[#1e1e3a]/80 rounded-lg px-3 py-1.5 gap-2 active:bg-[#2a2a4a] transition-colors">
          <Search size={16} className="text-gray-500 shrink-0" />
          <span className="text-gray-500 text-[13px]">Search</span>
          <span className="ml-auto text-gray-400 text-[12px] font-medium flex items-center gap-0.5">EN <ChevronRight size={12} className="text-gray-500 rotate-90" /></span>
        </button>
      )}
      <button className="flex items-center bg-blue-500 rounded-full pl-1.5 pr-2 py-1 gap-1 active:bg-blue-600 transition-colors shrink-0"
        onClick={() => comingSoon("Amino Coins")}>
        <span className="text-[12px]">🪙</span>
        <span className="text-white text-[12px] font-bold">{currentUser.coins}</span>
        <Plus size={12} className="text-white" />
      </button>
      <button onClick={() => comingSoon("Notifications")} className="p-1 relative active:scale-90 transition-transform shrink-0">
        <Bell size={22} className="text-white" fill="white" />
        <span className="absolute -top-0.5 -right-0.5 w-3 h-3 bg-red-500 rounded-full border-2 border-[#0f0f1e]" />
      </button>
    </motion.div>
  );
}
...
function CommunitiesTab() {
  const { communities, setSelectedCommunity, navigateTo, toggleCheckIn } = useApp();
  ...
  const handleTap = useCallback((c: Community) => {
    ...
    setSelectedCommunity(c);
    navigateTo("community");
  }, [setSelectedCommunity, navigateTo]);

  const handleCheckIn = useCallback((e: React.MouseEvent, c: Community) => {
    e.stopPropagation();
    if (!c.checkedIn) {
      toggleCheckIn(c.id);
      toast.success(`Checked in to ${c.name}!`);
    }
  }, [toggleCheckIn]);

  return (
    <motion.div {...fadeUp} className="overflow-y-auto amino-scroll px-3 pt-3 pb-4">
      <h2 className="text-white font-extrabold text-[22px] mb-4">My Communities</h2>
        ...
        <motion.div variants={stagger} initial="initial" animate="animate" className="grid grid-cols-3 gap-2">
          {joinedComms.map(c => (
            <motion.div key={c.id} variants={cardItem}
              ...
              <div className="relative rounded-xl overflow-hidden bg-[#16162a] aspect-[3/4]">
                {/* Full cover image */}
                <img src={c.cover} className="absolute inset-0 w-full h-full object-cover" alt="" />
                {/* Gradient overlay for text readability */}
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
                {/* Community icon - top left */}
                <div className="absolute top-2 left-2">
                  <img src={c.icon} className="w-8 h-8 rounded-lg object-cover border border-white/20 shadow-lg" alt="" />
                </div>
                {/* Notification badge - top right */}
                {(c.notifications ?? 0) > 0 && (
                  <div className="absolute -top-0.5 -right-0.5 z-10">
                    <span className="bg-red-500 text-white text-[10px] font-bold min-w-[18px] h-[18px] flex items-center justify-center rounded-full px-1 shadow-lg">
                      {(c.notifications ?? 0) > 9 ? "9+" : c.notifications}
                    </span>
                  </div>
                )}
                {/* Community name - bottom area */}
                <div className="absolute bottom-0 left-0 right-0 px-2 pb-1">
                  <p className="text-white font-bold text-[12px] text-center leading-tight mb-1.5"
                    style={{ textShadow: "0 1px 4px rgba(0,0,0,0.8), 0 0 8px rgba(0,0,0,0.5)" }}>
                    {c.name}
                  </p>
                  {/* Check In button */}
                  <button
                    onClick={(e) => handleCheckIn(e, c)}
                    className={`w-full py-1 rounded-md text-[11px] font-bold uppercase tracking-wide transition-all ${
                      c.checkedIn
                        ? "bg-gray-600/80 text-gray-300 cursor-default"
                        : "bg-[#2dbe60] text-white active:bg-[#25a854] hover:brightness-110"
                    }`}>
                    {c.checkedIn ? "\u2713 Done" : "Check In"}
                  </button>
                </div>
              </div>
            </motion.div>
