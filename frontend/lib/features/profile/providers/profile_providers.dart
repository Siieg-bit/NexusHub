...
// =============================================================================
// communityFrameProvider — Resolve a moldura local de um usuário em uma
// comunidade específica.
//
// Bug corrigido: o community_profile_screen usava equippedItemsProvider
// (que busca user_purchases WHERE is_equipped=true — moldura global) mesmo
// quando o usuário havia definido uma moldura específica para a comunidade
// via editor de perfil (active_avatar_frame_id em community_members).
//
// Lógica de prioridade:
//   1. Se community_members.active_avatar_frame_id != NULL → moldura local
//      (busca a URL via user_purchases JOIN store_items).
//   2. Se active_avatar_frame_id == NULL → retorna null, e o caller usa
//      equippedItemsProvider (moldura global) como fallback.
//
// Isso garante que a moldura definida no editor de perfil da comunidade
// sempre sobrescreva a moldura global definida pela loja.
// =============================================================================

/// Parâmetro composto para o provider (userId + communityId).
typedef CommunityFrameKey = ({String userId, String communityId});

/// Retorna {frame_url, frame_is_animated} da moldura local da comunidade,
/// ou null se o usuário não definiu uma moldura específica para a comunidade.
final communityFrameProvider =
    FutureProvider.family<Map<String, dynamic>?, CommunityFrameKey>(
        (ref, key) async {
  String _str(dynamic v) => v?.toString().trim() ?? '';
  String? _first(List<String> vals) {
    for (final v in vals) {
  ...
  try {
    // 1. Buscar active_avatar_frame_id do membership na comunidade
    final membership = await SupabaseService.table('community_members')
        .select('active_avatar_frame_id')
        .eq('user_id', key.userId)
        .eq('community_id', key.communityId)
        .maybeSingle();

    final activeFramePurchaseId =
        membership?['active_avatar_frame_id'] as String?;
    ...
    final si = purchase['store_items'] as Map<String, dynamic>?;
    if (si == null) return null;

    final ac = si['asset_config'];
    final assetConfig = ac is Map<String, dynamic>
        ? ac
        : (ac is Map ? Map<String, dynamic>.from(ac) : <String, dynamic>{});

    final frameUrl = _first([
      _str(assetConfig['frame_url']),
      _str(assetConfig['image_url']),
      _str(si['asset_url']),
      _str(si['preview_url']),
    ]);
    ...
    return {
      'frame_url': frameUrl,
      'frame_is_animated': assetConfig['is_animated'] as bool? ?? false,
    };
  } catch (_) {
    return null;
  }
